// api/games.js
// ESPN for schedule/teams/rankings
// Sidearm Sports for live stats URLs (school lookup table)

const fetch = require('node-fetch');
const { getCached, scrapeAll, SCHEDULE_PAGES } = require('./scrape');
...
const ESPN_SCOREBOARD = 'https://site.api.espn.com/apis/site/v2/sports/baseball/college-baseball/scoreboard';

async function fetchESPNGames(dateStr) {
  const url = `${ESPN_SCOREBOARD}?dates=${dateStr}&limit=200`;
...
function extractStatBroadcastId(competition) {
  const allLinks = [
    ...(competition.links || []),
    ...(competition.competitors || []).flatMap(c => c.links || []),
  ];
  for (const link of allLinks) {
    const href = link.href || '';
    const m = href.match(/statbroadcast\.com[^?]*\?.*id=(\d+)/i);
    if (m) return m[1];
  }
  return null;
}
...
function parseESPNEvent(event) {
  ...
  if (status?.type?.completed) gameStatus = 'final';
  else if (status?.type?.state === 'in') gameStatus = 'live';

  const detail = status?.type?.shortDetail || '';
  const inningMatch = detail.match(/(\d+)/);
  const currentInning = inningMatch ? parseInt(inningMatch[1]) : 1;
  const isTop = detail.toLowerCase().includes('top') || detail.toLowerCase().includes('t');

  const awayLS = (away.linescores || []).map(l => typeof l.value === 'number' ? l.value : null);
  const homeLS = (home.linescores || []).map(l => typeof l.value === 'number' ? l.value : null);
  const innings = Array.from({ length: 9 }, (_, i) => ({
    awayScore: awayLS[i] !== undefined ? awayLS[i] : null,
    homeScore: homeLS[i] !== undefined ? homeLS[i] : null,
  }));

  const getStat = (competitor, name) => {
    const s = (competitor.statistics || []).find(s => s.name === name || s.abbreviation === name);
    return s ? parseInt(s.displayValue) ?? null : null;
  };

  const sbId = extractStatBroadcastId(comp);
  const teamColor = (t) => t.team?.color ? `#${t.team.color}` : '#555555';
  ...
  return {
    ...
    away: {
      ...
      conf: away.team?.conferenceId || '',
      rank: away.curatedRank?.current <= 25 ? away.curatedRank?.current : null,
    ...
    home: {
      ...
      conf: home.team?.conferenceId || '',
      rank: home.curatedRank?.current <= 25 ? home.curatedRank?.current : null,
      record: home.records?.[0]?.summary || '',
    },
    status: gameStatus,
    inning: currentInning,
    half: isTop ? 'top' : 'bot',
    awayScore: gameStatus !== 'upcoming' ? parseInt(away.score) ?? null : null,
    homeScore: gameStatus !== 'upcoming' ? parseInt(home.score) ?? null : null,
    innings,
    awayHits: getStat(away, 'hits'),
    homeHits: getStat(home, 'hits'),
    awayErrors: getStat(away, 'errors'),
    homeErrors: getStat(home, 'errors'),
    outs: 0,
    bases: [false, false, false],
    time: gameStatus === 'upcoming' ? event.date : null,
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ success: true, timestamp: new Date().toISOString(), gameCount: games.length, games });
  } catch (err) {
